...
class Etablissement {
    public static function getSingleId() {
        try {
            $db = getDB();
            $stmt = $db->query("SELECT id FROM etablissement LIMIT 1");
...
public static function addFonds($id, $montant) {
    try {
        ...
        $stmt = $db->prepare("INSERT INTO etablissement (fonds_disponibles, date_ajout) VALUES (?, CURRENT_TIMESTAMP)");
...
public static function getSolde() {
    try {
        $db = getDB();
        $stmt = $db->query("SELECT SUM(fonds_disponibles) AS total FROM etablissement");
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
